...
Vertex(:,1)=[-sqrt(3)*r/2,-sqrt(3)*r/2,0,0,sqrt(3)*r/2,sqrt(3)*r/2,sqrt(3)*r/2,sqrt(3)*r/2,-sqrt(3)*r/2,-sqrt(3)*r/2,0,0];
Vertex(:,3)=[-r/2,r/2,r/2,-r/2,-r/2,r/2,r/2,-r/2,-r/2,r/2,r/2,-r/2];
vp=[Vertex(1:4,:);Vertex(1,:)];
firstpoint(1,:)=vp(2,:);
direct_vactor=zeros(8,9);
direct_vactor(1,:)=[vp(3,:)-vp(2,:),vp(1,:)-vp(2,:),[0 0 0]];


fill3(vp(:,1),vp(:,2),vp(:,3),'g-')
alpha(0.3)
M1=mean(vp);
 text(M1(1),M1(2),M1(3),num2str(1),'HorizontalAlignment','left','FontSize',12);
hold on;
vp=[Vertex(5:8,:);Vertex(5,:)];
firstpoint(3,:)=vp(3,:);
direct_vactor(3,:)=[vp(2,:)-vp(3,:),vp(1,:)-vp(2,:),[0 0 0]];
fill3(vp(:,1),vp(:,2),vp(:,3),'g-')
alpha(0.3)
M1=mean(vp);
 text(M1(1),M1(2),M1(3),num2str(3),'HorizontalAlignment','left','FontSize',12);
hold on;
vp=[Vertex(9:12,:);Vertex(9,:)];
firstpoint(5,:)=vp(3,:);
direct_vactor(5,:)=[vp(2,:)-vp(3,:),vp(1,:)-vp(2,:),[0 0 0]];
fill3(vp(:,1),vp(:,2),vp(:,3),'g-')
alpha(0.3)
M1=mean(vp);
 text(M1(1),M1(2),M1(3),num2str(5),'HorizontalAlignment','left','FontSize',12);
hold on;
vp=[Vertex(3:4,:);Vertex(8:-1:7,:);Vertex(3,:)];
firstpoint(2,:)=vp(2,:);
direct_vactor(2,:)=[vp(3,:)-vp(2,:),vp(2,:)-vp(1,:),[0 0 0]];
fill3(vp(:,1),vp(:,2),vp(:,3),'g-')
alpha(0.3)
M1=mean(vp);
 text(M1(1),M1(2),M1(3),num2str(2),'HorizontalAlignment','left','FontSize',12);
hold on;
vp=[Vertex(5:6,:);Vertex(11:12,:);Vertex(5,:)];
firstpoint(4,:)=vp(2,:);
direct_vactor(4,:)=[vp(3,:)-vp(2,:),vp(1,:)-vp(2,:),[0 0 0]];
fill3(vp(:,1),vp(:,2),vp(:,3),'g-')
alpha(0.3)
M1=mean(vp);
 text(M1(1),M1(2),M1(3),num2str(4),'HorizontalAlignment','left','FontSize',12);
hold on;
vp=[Vertex(1:2,:);Vertex(10:-1:9,:);Vertex(1,:)];
firstpoint(6,:)=vp(3,:);firstpoint(7,:)=Vertex(1,:);firstpoint(8,:)=Vertex(2,:);
direct_vactor(6,:)=[vp(2,:)-vp(3,:),vp(1,:)-vp(2,:),[0 0 0]];
direct_vactor(7,:)=[Vertex(6,:)-Vertex(7,:),Vertex(3,:)-Vertex(2,:),Vertex(7,:)-Vertex(3,:)];
direct_vactor(8,:)=direct_vactor(7,:);

fill3(vp(:,1),vp(:,2),vp(:,3),'g-')
alpha(0.3)
M1=mean(vp);
 text(M1(1),M1(2),M1(3),num2str(6),'HorizontalAlignment','left','FontSize',12);
 M1=[0,0 -r/2];
 text(M1(1),M1(2),M1(3),num2str(7),'HorizontalAlignment','left','FontSize',12,'rotation',-90);
 M1=[0,0 r/2];
 text(M1(1),M1(2),M1(3),num2str(8),'HorizontalAlignment','left','FontSize',12,'rotation',90);
hold on;
xlabel('x axis')
ylabel('y axis')
zlabel('z axis')
axis([-r-20 r+20 -r-20 r+20 -r-20 r+20]);
grid on;
drawnow
hold on;
...
normal_vector(8,:)=[0 0 1];b(8)=normal_vector(8,:)*Vertex(2,:)';
...
while(num<num_rays)
    p=[];
    for x=-span*(16):span:span*15
        for y= -span*(16):span:span*15
            p=[p;[x,y,-r*1.2]];
        end
    end
    for i=1:size(p,1)
        ...
        if(~isempty(p1))
            ...
            for k=1:length(Pn)
                ...
                if(k==1)
                    ...
                    dv=direct_vactor(wallnum(k),1:3);
                    dv=dv./norm(dv);
                    tp01=tp0*dv';
                    tplen=sqrt(tp0*tp0');
                    tp2=tp01/tplen;
                    ...
                    dv=direct_vactor(wallnum(k-1),1:3);
                    dv=dv./norm(dv);
                    tp01=tp0*dv';
                    tplen=sqrt(tp0*tp0');
                    tp2=tp01/tplen;
